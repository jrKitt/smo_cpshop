...
function showSelectionDetails() {
  ...
  const jerseySizeSection = document.getElementById("jersey-size-section");
  ...
  jerseySizeSection.style.display = "none";
  individualItemsSection.style.display = "none";
  poloSizesContainer.innerHTML = "";
  jacketSizesContainer.innerHTML = "";
  const jerseySizesContainer = document.getElementById(
    "jersey-sizes-container"
  );
  if (jerseySizesContainer) jerseySizesContainer.innerHTML = "";
...
function updateSizeSelectors(type) {
  ...
  for (let i = 0; i < quantity; i++) {
    ...
    let itemName = "เสื้อโปโล";
    if (type === "jacket") itemName = "เสื้อแจ็คเก็ต";
    if (type === "jersey") itemName = "เสื้อ JERSEY";
    label.textContent = `${itemName} ตัวที่ ${i + 1}:`;
...
function updateIndividualItems() {
  const newQuantities = {
    polo: parseInt(document.getElementById("polo-select").value) || 0,
    jacket: parseInt(document.getElementById("jacket-select").value) || 0,
    jersey: parseInt(document.getElementById("jersey-select").value) || 0,
    belt: parseInt(document.getElementById("belt-select").value) || 0,
    tung: 0,
    tie: parseInt(document.getElementById("tie-select").value) || 0,
  };

  const shirtQuantity =
    newQuantities.polo + newQuantities.jacket + newQuantities.jersey;

  if (shirtQuantity > 3) {
    Swal.fire({
      icon: "warning",
      title: "เกินจำนวนเสื้อที่กำหนด",
      text: "เสื้อ (โปโล + แจ็คเก็ต + เจอร์ซี่) รวมกันสั่งได้ไม่เกิน 3 ชิ้น",
      confirmButtonText: "ตกลง",
    });

    const shirtSelects = ["polo-select", "jacket-select", "jersey-select"];
    shirtSelects.forEach((selectId) => {
      const select = document.getElementById(selectId);
      if (select && select === document.activeElement) {
        const itemName = selectId.replace("-select", "");
        select.value = individualItems[itemName].quantity;
      }
    });
    return;
  }
  ...
  if (totalQuantity > 3) {
    Swal.fire({
      ...
      text: "คำสั่งซื้อหนึ่งครั้งสั่งได้ไม่เกิน 3 ชิ้นรวมทั้งหมด",
      confirmButtonText: "ตกลง",
    });
  ...
  individualItems.polo.quantity = newQuantities.polo;
  individualItems.jacket.quantity = newQuantities.jacket;
  individualItems.jersey.quantity = newQuantities.jersey;
  individualItems.belt.quantity = newQuantities.belt;
  individualItems.tung.quantity = 0;
  individualItems.tie.quantity = newQuantities.tie;

  updateSizeSelectors("polo");
  updateSizeSelectors("jacket");
  updateSizeSelectors("jersey");
  ...
  if (individualItems.jersey.quantity > 0)
    packages["individual"].items.push("เสื้อ JERSEY");
...
function updateShirtCounter() {
  const shirtCount =
    ...
    individualItems.jersey.quantity;

  const counterElement = document.getElementById("shirt-count");
  const badgeElement = document.getElementById("shirt-counter");

  if (counterElement) {
    counterElement.textContent = shirtCount;
  }
...
function proceedToCheckout() {
  ...
  if (selectedPackage === "individual") {
    ...
    const totalQuantity = Object.values(individualItems).reduce(
    ...
    if (totalQuantity > 3) {
      Swal.fire({
        ...
        text: "คำสั่งซื้อหนึ่งครั้งสั่งได้ไม่เกิน 3 ชิ้น",
        confirmButtonText: "ตกลง",
      });
      return;
    }
    ...
    if (individualItems.jersey.quantity > 0) {
      const allJerseySizesSelected = selectedSizes.jersey.every(
        (size) => size !== null
      );
      if (!allJerseySizesSelected) {
        alert("กรุณาเลือกไซส์เสื้อ JERSEY ให้ครบทุกตัว");
        return;
      }
    }
  } else {
...
function generateEmailHTML(orderData) {
  ...
  if (selectedPackage === "individual") {
    ...
    if (orderData.items.jacket > 0) {
      emailHTML += `<div style="margin-bottom: 10px;"><strong style="color: #333;">เสื้อแจ็คเก็ต:</strong> <span style="color: #666;">${orderData.items.jacket} ชิ้น</span></div>`;
      for (let i = 0; i < orderData.items.jacket; i++) {
        if (orderData.sizes.jacket && orderData.sizes.jacket[i]) {
          emailHTML += `<div style="margin-left: 20px; margin-bottom: 5px; color: #666;">- ตัวที่ ${
            i + 1
          }: ไซส์ ${sanitizeText(orderData.sizes.jacket[i])}</div>`;
        }
      }
    }
    if (orderData.items.jersey > 0) {
      emailHTML += `<div style="margin-bottom: 10px;"><strong style="color: #333;">เสื้อ JERSEY:</strong> <span style="color: #666;">${orderData.items.jersey} ชิ้น</span></div>`;
      for (let i = 0; i < orderData.items.jersey; i++) {
        if (orderData.sizes.jersey && orderData.sizes.jersey[i]) {
          emailHTML += `<div style="margin-left: 20px; margin-bottom: 5px; color: #666;">- ตัวที่ ${
            i + 1
          }: ไซส์ ${sanitizeText(orderData.sizes.jersey[i])}</div>`;
        }
      }
    }
  } else {
...
function generateItemsSummary() {
  return {
    ...
    jersey: individualItems.jersey.quantity,
...
async function uploadSlipViaServer(file, base64Data, orderRef) {
  try {
    const uploadData = {
      file: base64Data,
...
async function submitOrder() {
  ...
  if (selectedPackage === "individual") {
    ...
    if (orderData.items.jersey > 0)
      summaryText += `เสื้อ JERSEY: ${orderData.items.jersey} ชิ้น\n`;
...
function showSuccessPage(orderData) {
  ...
  if (selectedPackage === "individual") {
    ...
    // เพิ่มสรุปสำหรับรายการอื่น ๆ
    if (orderData.items.jacket > 0) {
      const jacketP = document.createElement("p");
      jacketP.textContent = `เสื้อแจ็คเก็ต: ${orderData.items.jacket} ชิ้น`;
      summaryDiv.appendChild(jacketP);
      for (let i = 0; i < orderData.items.jacket; i++) {
        if (orderData.sizes.jacket && orderData.sizes.jacket[i]) {
          const sizeP = document.createElement("p");
          sizeP.className = "ms-3";
          sizeP.textContent = `- ตัวที่ ${i + 1}: ไซส์ ${
            orderData.sizes.jacket[i]
          }`;
          summaryDiv.appendChild(sizeP);
        }
      }
    ...
    if (orderData.items.jersey > 0) {
      const jerseyP = document.createElement("p");
      jerseyP.textContent = `เสื้อ JERSEY: ${orderData.items.jersey} ชิ้น`;
      summaryDiv.appendChild(jerseyP);
      for (let i = 0; i < orderData.items.jersey; i++) {
        if (orderData.sizes.jersey && orderData.sizes.jersey[i]) {
          const sizeP = document.createElement("p");
          sizeP.className = "ms-3";
          sizeP.textContent = `- ตัวที่ ${i + 1}: ไซส์ ${
            orderData.sizes.jersey[i]
          }`;
          summaryDiv.appendChild(sizeP);
        }
      }
  ...
  if (orderData.trackingCode) {
    ...
    trackingP.innerHTML = `<strong>รหัสติดตาม:</strong> <span class="badge bg-primary">${orderData.orderRef}</span>`;
    summaryDiv.appendChild(trackingP);
  }
  ...
  try {
    ...
    fetch("/submit-order", {
      ...
      .then((data) => {
        hideLoadingMessage();
        if (data.success) {
          orderData.orderRef = data.orderRef;
          orderData.trackingCode = data.trackingCode;
...
// เรียกใช้ค่าเริ่มต้นให้แสดงสินค้าแยกชิ้น
document.addEventListener("DOMContentLoaded", () => {
  filterProducts("individual");
});
